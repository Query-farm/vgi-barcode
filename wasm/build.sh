#!/usr/bin/env bash
# Build the browser (`worker:`) build of the barcode VGI worker: barcode-wasm
...
LIB="target/$TARGET/release/libbarcode_wasm.a"
...
# target only; it does not change the native build.
#
...
# which this worker overflows — the decoders + Arrow encoders nest deeply, and an
# overflow surfaces as a bare "memory access out of bounds" with no console
# output. The serve threads actually run the work, so the pthread stack matters
# most. INITIAL_MEMORY must cover main stack + every pthread stack up front, or
# startup stalls trying to grow shared memory while the pool is spawning.
echo "==> emcc link"
