...
#
# Dispatch the single vgi-barcode image into one of its transports:
...
#
# The worker is stateless (barcode decode/encode is pure per-request compute),
...
case "${1:-http}" in
  http)
    ...
    exec barcode-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec barcode-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec barcode-worker "$@"
