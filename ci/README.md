# CI: the vgi-barcode worker integration suite
...
(`test/sql/*.test`) against the vgi-barcode VGI worker through the **real DuckDB
...
1. **Build the worker** — `cargo build --release --bin barcode-worker`. The
   compiled `target/release/barcode-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_BARCODE_WORKER}`).
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, copying the committed `test/sql/data/` fixtures (which the `.test` files read by relative path), points `VGI_BARCODE_WORKER` at the release binary, warms the
...
```bash
cargo build --release --bin barcode-worker
HAYBARN_UNITTEST=/path/to/haybarn-unittest \
VGI_BARCODE_WORKER="$PWD/target/release/barcode-worker" \
