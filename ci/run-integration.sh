...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-barcode
...
# is what LOCATION (VGI_BARCODE_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_BARCODE_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `barcode-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_BARCODE_WORKER = http://127.0.0.1:<port>.
#               If VGI_BARCODE_WORKER is ALREADY set to an http(s):// URL (e.g. a
#               pre-launched container in the docker image_test), it is used
#               as-is and no local worker is spawned.
#   unix        start `barcode-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_BARCODE_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled barcode-worker binary (used to launch
#                     the http/unix servers, and the stdio LOCATION). Falls back
#                     to VGI_BARCODE_WORKER when that is a bare command (subprocess).
# Optional:
...
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/barcode-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_BARCODE_WORKER.
...
# worker is launched with cwd = the stage dir so it resolves staged relative
# fixtures (test/sql/data/*) identically to the subprocess (DuckDB-spawned) case.
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the barcode-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_BARCODE_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_BARCODE_SOCK:-/tmp/barcode.$$.sock}"
      ...
      export VGI_BARCODE_WORKER="unix://$SOCK_PATH"
...
# The .test files read committed fixture images by the relative path
# test/sql/data/*; the worker cwd is the stage dir, so copy them in.
...
case "$TRANSPORT" in
  subprocess) export VGI_BARCODE_WORKER="${VGI_BARCODE_WORKER:-$WORKER_BIN}" ;;
  http)
    # Honor a pre-launched HTTP worker (e.g. a running container in the docker
    # image_test): if VGI_BARCODE_WORKER already points at an http(s) URL, use it
    # and skip spawning a local binary. The awk preprocessor still injects httpfs
    # because TRANSPORT=http.
    if [[ "${VGI_BARCODE_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_BARCODE_WORKER"
    else
...
: "${VGI_BARCODE_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_BARCODE_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_BARCODE_WORKER." >&2
  exit 1
fi
exit "$status"
