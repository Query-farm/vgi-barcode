...
#   VGI_BARCODE_WORKER  worker LOCATION the .test files attach (a stdio command
#                     such as the compiled barcode-worker binary, or an http:// URL)
# Optional:
#   STAGE             scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_BARCODE_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
# The .test files read committed fixtures by the relative path test/sql/data/*;
# the worker cwd is the stage dir, so copy them in.
...
echo "Running suite (worker: $VGI_BARCODE_WORKER) ..."
