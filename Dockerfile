...
#
# Single image that serves every network transport of the `barcode` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: barcode decode/encode is pure
# computation over the request BLOB (rxing + a raster codec compiled into the
# binary), so there is no /data volume, no registry, and no
...
# runtime ships. All dependencies (vgi, rxing, image) resolve straight from
# crates.io — no git patch, so no extra tooling is needed beyond the rust image.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin barcode-worker \
    && cp target/release/barcode-worker /barcode-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-barcode
...
LABEL org.opencontainers.image.title="vgi-barcode" \
      org.opencontainers.image.description="Barcode & QR-code decoding and generation as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_BARCODE_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /barcode-worker /usr/local/bin/barcode-worker
