//! The `barcode` VGI worker (library).
//!
...
//! the `barcode-wasm` crate (the browser build, which serves the same `Worker`
...
/// Build the fully-registered `barcode` worker: register every scalar and table
/// function, attach the catalog metadata, and return the `Worker` ready to serve.
///
/// Both entrypoints call this and differ only in transport: the native binary
/// (`main.rs`) calls `.run()`, while the browser build (`barcode-wasm`) serves it
/// over a SharedArrayBuffer byte channel.
