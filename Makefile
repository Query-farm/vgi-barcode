# vgi-barcode worker — dev, test, and lint targets.
#
# Usage:
...
WORKER         ?= $(CURDIR)/target/release/barcode-worker
...
test-sql: build
	VGI_BARCODE_WORKER="$(WORKER)" $(SQL_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"
...
fixtures:
	cargo run -p barcode-worker --example gen_fixtures
